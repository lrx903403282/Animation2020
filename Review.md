...
即使有大量现成的高质量运动捕捉数据，为虚拟角色生成实时数据驱动的动作仍是一个具有挑战性的工作。而该工作的挑战性在于，作为一个动作生成系统，其需要具备从数据中学习的能力，不需要太多的手工数据预处理，在运行时能够速度快而且占用内存少。而当情况变得更加复杂时，如地形不平坦且增加了各种障碍物、角色需要执行如攀爬、跳跃、回避等动作、角色与环境需要更多的交互时，实现动作生成系统将更加困难。本文通过查阅相关文献，分析了各种动作生成模型的优势与局限，同时以2017、2018、2019年的基于神经网络的动作生成模型PFNN、MANN、NSM作为里程碑，进行了重点分析，为我们日后的工作提供了参考。
...
通过试运行作者提供的训练结果，我们为这一模型的高拟真度所震撼。作者在2019年发表的论文中指出，神经状态机依然有一定的局限性，如训练结果高度依赖数据集的丰富程度、数据集的捕捉依赖人工经验等。我们发现，作者在2018和2019年分别提出的MANN和NSM在应用层面的结合点，即人与动物的交互动画的生成，依赖于神经网络中动作标签和权重系统范围的扩大，以及数据集丰富度的提升。我们希望在这里进行较为深入的探讨，并基于提供的模型综合出一个适用于包含人类与动物角色交互动画的神经网络模型，产出具有应用价值的成果。
